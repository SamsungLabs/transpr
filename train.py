...
def run_epoch(pipeline, phase, epoch, args, iter_cb=None):
    ...
    def run_sub(dl, extra_optimizer):
        ...
        for it, data in enumerate(dl):
            ...
            if mask is not None and args.loss_bck_weight>0:
                loss /= mask.mean() + 1e-6
                ...
                loss_bkg = args.loss_bck_weight * torch.abs((out_bkg - bkg)).mean() / (n_mask.mean() + 1e-6)

                loss += loss_bkg

                ad.add('loss_bkg', loss_bkg.item())

            if hasattr(pipeline.model, 'reg_loss') and args.reg_weight>0:
    ...
    def on_iter(self, it, max_it, input, out, target, data_dict, ad, phase, epoch):    
        ...
        if it % self.args.log_freq_images == 0:
            if isinstance(out, dict):
                out_img = out['net_output']
                if 'target' in out:
                    target = out['target']
                if 'inputs' in out and self.args.raster_alpha_to_tb:
                    out_alpha = out['inputs'][self.args.tb_mipmap_scale][:,[-1]] # rasterized alpha visualization
                    out_alpha = F.interpolate(out_alpha, size=target.shape[2:])

                    out_img = torch.cat([out_img, out_alpha], dim=1)
            else: 
                out_img = out
            out_img = out_img.clamp(0, 1)
            self.writer.add_image(f'{phase}', image_grid(out_img, target), self.train_it)
...
if __name__ == '__main__':
    ...
    parser.add('--pipeline', type=str, default='npbg.pipelines.ray.RayPipeline', help='path to pipeline module')
    ...
    parser.add('--loss_bck_weight', type=float, default=0.)
    ...
    parser.add('--raster_alpha_to_tb', action='store_bool', default=True, help='log rasterized alpha into tensorboard')    
    parser.add('--tb_mipmap_scale', type=int, default=0, help='resolution to log into tensorboard from [0, num_mipmap)')
